...
// CUSTOM: tombstone rotary WCS - returns the abc Vector to use for the program's
// opening positionABC() call (first section). Honors tombstoneRotaryInitial only;
// the per-WCS rank offset is applied later by applyTombstoneRotaryOffset() when
// the section's workplane is established.
function getTombstoneOpeningABC() {
  if (tombstoneRotaryCoord < 0) {
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel || state.tcpIsActive || currentSection.isMultiAxis()) {
    ...
    if (isFirstSection()) {
      ...
      if (machineConfiguration.isMultiAxisConfiguration()) {
        // CUSTOM: tombstone rotary WCS - open the program at the configured
        // initial rotary offset instead of B0, so a program running on a
        // station that is not at the rotary home position lines up correctly.
        positionABC(getTombstoneOpeningABC());
      }
      forceABC();
    } else {
...
var sizeTables = {
  "drill": [
    ...
    { "name": "21/32 in"  , "inch": 0.6563, "mm": 16.6688 },
...
// CUSTOM: shared scaffolding for the canned drilling cycles. Every cycle type
// emits the same G71 clearance line followed by a single cycle block: the plane,
// an optional retract mode (G98/G99), the cycle G-code, the common X/Y/Z/R words,
// the type-specific extra words, and the trailing M53. Only the G-code, retract
// mode and extra words differ between types, so the switch in writeDrillCycle is
...
function writeDrillCycle(cycle, x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    function pWordMs() { return conditional(P > 0, "P" + milliFormat.format(P)); }
    // The common X/Y/Z/R cycle words. Called lazily (only by the canned cases, not
    // the expanded default path) because getCommonCycle() has side effects -
    // forceXYZ() and xyz formatting - that must NOT run for an expanded cycle.
    function cc() { return getCommonCycle(x, y, z, cycle.retract, cycle.clearance); }
    var F = scaleFeed(cycle.feedrate);
    var P = !cycle.dwell ? 0 : clamp(1, cycle.dwell * 1000, 99999999); // in milliseconds
    // Format the G71 clearance word once, unconditionally, before the switch. This
    // call mutates z71Output's modal state, which the expanded (default) path also
    // relies on, so it must run for every cycle type.
    ...
    if (section.getId() != sectionId) {
      ...
      if (subprogramType == SUB_PATTERN) {
        if (section.getPatternId() == subprogramId) {
          // CUSTOM: reuse multi-WCS subprograms - bypass the world-frame box
          // checks (which fail for rotary-tombstone WCS clones) and accept the
          // section as a valid incremental pattern clone. The subprogram body
          // is emitted in G91 so each CALL works regardless of starting frame.
          // Probing operations are excluded: their macro calls bake the work
          // offset (PS=<probeWorkOffset>) into the subprogram body, which is
          // only correct for one WCS.
          if (getProperty("reuseMultiWCSSubprograms")) {
            if (section.isMultiAxis() || isTCPSupportedByOperation(section) ||
                _section.getTool().type == TOOL_PROBE) {
