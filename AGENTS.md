...
- **Re-grouped post properties with `groupDefinitions` for sidebar layout.**
  ...
  - Group keys → display titles → property keys (in display order):
    ...
    3. `spindle` "Spindle & C-Axis": `turningModeCommand`, `useGearRanges`, `optimizeCAxisSelect`, `useShortestDirection`, `useCAxisClamping`.
    4. `cycles` "Cycles, Feeds & Arcs": `useCycles`, `feedPerRevForDrilling`, `useSimpleThread`, `useSimpleCycles`, `useYAxisForDrilling`, `useParametricFeed`, `useRadius`.
    5. `barPuller` "Bar Puller": `useToolBarPuller`, `toolBarPullerNumber`, `barPullerZOffset`.
    6. `stockHandling` "Stock Handling": `usePartCatcher`, `autoEject`, `transferUseTorque`.
    7. `programBehavior` "Program Behavior": `optionalStop`, `safeStartAllOperations`, `loadMonitoring`.
    8. `partLooping` "Part Looping (Bar Feeder)": `loopForMultipleParts`, `partCounterVariable`, `partsPerBar`, `partsPerBarVariable`.
    9. `output` "Program Output & Formatting": `showSequenceNumbers`, `sequenceNumberStart`, `sequenceNumberIncrement`, `separateWordsWithSpace`, `writeVersion`, `writeMachine`, `writeTools`, `showNotes`.
  - `groupDefinitions` is the standard Autodesk Post API mechanism for naming and ordering property groups. If a particular Fusion build doesn't honor it, the properties still work -- they'd just be shown grouped by the raw key (`machine`, `homePositions`, etc.) in alphabetical order, with no group descriptions.
...
- **Tool-based bar puller (no secondary spindle required).**
  ...
  - Emitted sequence (in `writeToolBarPullerCycle`):
    ...
    5. Rapid `Z<gripZ>` then rapid `X<stockDiameter>` (part WCS frame). Z-first is safe because we start from home (well clear of the bar in X) and approach from outside the stock OD -- positioning Z axially first avoids any chance of dragging the puller body along the bar OD during the approach.
...
- **Configurable turning-mode entry code (G270 / M109 / None).**
  ...
  - Property (group `spindle`):
    - `turningModeCommand` (enum, default `"g270"`) - one of:
      ...
      - `"m109"` -> emit `M109` (DISABLE_C_AXIS) via `cAxisEngageModal`. Use on the LB15-II.
      - `"none"` -> emit nothing.
  ...
  - **M109 paired with M110 inside each live-tool cycle.** M109/M110 emission is deduped via a dedicated boolean state tracker `cAxisIsEngaged` (declared near the top of the file, reset to `false` in `onOpen` -- program is assumed to start in M109 / turning mode). Two helpers mediate every state change:
    - `cAxisEnableWord()` returns `mFormat.format(getCode("ENABLE_C_AXIS", ...))` (i.e. `"M110"`) and sets the flag to `true` when the C-axis isn't yet engaged; returns `""` when already engaged. Used inline in `writeBlock(feedMode, gPlaneModal.format(plane), cAxisEnableWord())` in the live-tool branch of `onSection`, which preserves the original "G95 G19 M110" combined-on-one-line format and emits nothing when M110 was already in effect.
    - `cAxisDisableWord()` is the M109 mirror -- returns `"M109"` (and flips the flag) when engaged, `""` otherwise. Called from `writeTurningModeEntry()` (`"m109"` mode), from `onSectionEnd`'s live-exit branch, and from `ejectPart()`.
    The flag is the single source of truth -- it's set/cleared only by these two helpers, so it always matches what the post actually emitted. This replaces an earlier modal-based scheme that used `cAxisEngageModal.reset()` + `cAxisEngageModal.format(...)`; the stock post sprinkled `.reset()` calls (gated on `!getProperty("optimizeCAxisSelect")`, which defaults off so they always fired) at every section start, every tool change, and in `ejectPart`, defeating modal dedup and causing redundant M109/M110 on every section. All four of those `.reset()` calls have been removed and the corresponding `.format(...)` call sites switched to the boolean helpers (one dead-code `.reset()+force-emit` branch in the previous-live-to-live transition was also dropped). The `cAxisEngageModal = createOutputVariable({}, mFormat)` declaration remains at file scope as a no-op for now.
    Emission sites:
    - **Entry into live tool**: live-tool branch in `onSection` calls `cAxisEnableWord()` -- emits `M110` on the same line as feedMode + plane.
    - **Exit from live tool**: `onSectionEnd` emits `writeBlock(cAxisDisableWord())` when `cAxisIsEngaged` is true (and `!machineState.stockTransferIsActive`), with one skip: if the next section uses the same tool number (`getNextSection().getTool().number == tool.number`), stay in M110 across the boundary. The flag-based check replaces an earlier `getSpindle(TOOL) == SPINDLE_LIVE` check, which silently returned the next section's spindle at `onSectionEnd` time and dropped the emission.
    - **Safety net at turning section entry**: `writeTurningModeEntry()` (called from the previous-live-to-turning transition in `onSection` and at the start of every turning section's setup block) emits `M109` via `cAxisDisableWord()` -- normally a no-op because the prior live section's `onSectionEnd` already emitted it, but covers any path where the C-axis is somehow still engaged.
    - **Safety net at program end**: same `writeTurningModeEntry()` is called from `onClose` after the final retract -- emits `M109` only if the program ended with the C-axis still engaged (e.g. all-live program), no-op otherwise.
  - Marker comment: `// CUSTOM: configurable turning-mode entry code` (property block), `// CUSTOM: M109/M110 state tracker` (`cAxisIsEngaged` declaration), `// CUSTOM: M109/M110 emission helpers` (the two helpers), `// CUSTOM: M109 emission is deduped via the cAxisIsEngaged state tracker` (`writeTurningModeEntry` body), `// CUSTOM: M110 emission is deduped via cAxisIsEngaged` (live-tool `onSection` site), `// CUSTOM: cAxisEngageModal.reset() removed` (the three removed-reset sites in `onSection` / `ejectPart`), `// CUSTOM: dead-code cAxisEngageModal.reset()+force-emit removed` (live-to-live inner branch), `// CUSTOM: exit C-axis mode (M109) at the end of any live-tool section` (in `onSectionEnd`), `// CUSTOM: program is assumed to start with the C-axis disengaged` (in `onOpen`), `// CUSTOM: emit M109 only if the C-axis is currently engaged` (in `ejectPart`).
...
- **Paired commands stay inside their cycle, deduped across sections.**
  ...
  - Changes (all in addition to the **Output cleanup** entry above):
    ...
    7. **SFM-cancel `startSpindle` gated around the bar-pull cycle.** The stock post's "cancel SFM mode to preserve spindle speed" block at the bottom of `onSectionEnd` calls `startSpindle(false, true, ...)` whenever the section's tool is in `SPINDLE_CONSTANT_SURFACE_SPEED` mode, to swap G96 -> G97 and freeze RPM for the next section. Two cases produced bogus output: (a) at the end of the section *before* a tool-based bar pull, our prelude has just emitted `M5 / M9 / retract / M01` -- the SFM-cancel block then re-emitted `G97 S<n> M03`, restarting the spindle *after* the `M01` and right before the bar-pull body (which must run with the spindle stopped). (b) At the end of the bar-pull section itself, the SFM-cancel block fired again because the puller "tool" in Fusion has `SPINDLE_CONSTANT_SURFACE_SPEED` set on its tool definition -- producing a stray `G97 S<maxRPM>` after the bar-pull's own `M01`, followed by an `M05` from onClose's stop-spindle. The fix extends the existing `!machineState.stockTransferIsActive` guard with two new flags: skip when `barPullPreludeEmitted` is true (the next section is a bar pull -- spindle must stay off) and when `barPullWroteOwnWrapup` is true (the current section *is* a tool-based bar pull that already closed itself out).
  ...
  - Marker comments: `// CUSTOM: skip M05/M12 when the relevant spindle isn't currently running` (`onCommand(COMMAND_STOP_SPINDLE)` dedup), `// CUSTOM: snapshot whether the relevant spindle is already running` (`startSpindle` snapshot), `// CUSTOM: suppress the M03/M04 spindle-direction word` (`startSpindle` dedup site), `// CUSTOM: stop the main spindle too` (the new `else` branch in `onSection`'s wrap-up), `// CUSTOM: run polar / Y-axis teardown BEFORE the bar-pull prelude` (`onSectionEnd` reorder), `// CUSTOM: tool-based bar pull doesn't use a canned drilling cycle` (`onCycleEnd` early-return), `// CUSTOM: skip this block when the spindle has just been stopped as part of a bar-pull transition` (the SFM-cancel gate in `onSectionEnd`).

- **Tailstock engagement driven entirely by Fusion's per-operation checkbox.**
  - The stock post had a `useTailStock` post property that gated whether *any* `M21` / `M22` was emitted, on top of Fusion's per-operation "Use tailstock" checkbox (`currentSection.tailstock`). That double gate was redundant -- if the operation doesn't want the tailstock, the checkbox handles it; if the machine has no tailstock, simply never check the box. The post property has been removed.
  - The stock post also emitted the tailstock M-code (both engage `M21` and disengage `M22`) inside the new section's body, AFTER the section header / tool change / spindle start. So the `M22` that closes out a tailstock-using cycle visually belonged to the *next* operation rather than the one it was actually wrapping up.
  - Fix: split engage and disengage into two emission sites.
    - **Engage (`M21`)** stays in `onSection`, right after the manual-tool-change block. Simplified to: if `currentSection.tailstock` AND not `currentSectionTailstockIncompatible()` AND not already engaged, emit `M21`. Incompatible sections (sub-spindle, axial-center drilling, axial live-tool milling) trigger the existing warning instead. No post-property gate.
    - **Disengage (`M22`)** is emitted from three sites, all inside the prior section's wrap-up:
      1. The wrap-up branch at the top of `onSection` (the normal section-transition path): between `COMMAND_COOLANT_OFF` and `writeRetract(X)`. Emits `M22` only when `machineState.tailstockIsActive` AND the new `currentSection` either doesn't want tailstock OR is incompatible. When the next section *also* wants the tailstock and is compatible, no `M22` (and no later `M21`) emits -- the tailstock stays engaged across the boundary.
      2. The bar-pull prelude in `onSectionEnd` (when the next section is a tool-based bar pull): unconditional `M22` when active, since bar-pull doesn't use the tailstock.
      3. `onClose` (existing): catches the case where the final section ended with the tailstock still engaged.
  - Helper `currentSectionTailstockIncompatible()` (placed next to `cAxisDisableWord`) returns true when `machineState.axialCenterDrilling || getSpindle(PART) == SPINDLE_SUB || (getSpindle(TOOL) == SPINDLE_LIVE && machineState.machiningDirection == MACHINING_DIRECTION_AXIAL)`. Relies on `updateMachiningMode(currentSection)` having been called earlier in `onSection` (it has, at line ~1806).
  - Net effect on a tailstock-engaged cycle: was `... / M5 / M9 / G0 X<home> / Z<home> / M01 / <blank> / (NEXT) / N# T<n> / ... / M22`; now `... / M5 / M9 / M22 / G0 X<home> / Z<home> / M01 / <blank> / (NEXT) / N# T<n> / ...`. The `M22` visually belongs to the cycle that no longer needs the tailstock.
  - Marker comments: `// CUSTOM: useTailStock removed` (removed property site), `// CUSTOM: tailstock engage only` (engage block in `onSection`), `// CUSTOM: tailstock disengage lands in the prior section's wrap-up` (wrap-up branch in `onSection`), `// CUSTOM: release the tailstock here too` (bar-pull prelude in `onSectionEnd`), `// CUSTOM: returns true if the current section is incompatible with the tailstock` (helper).

- **Part-loop mode for bar-feeder runs.**
  - Goal: when running with a bar feeder, post a single program that machines N parts off the bar without operator intervention. Track completed parts in an Okuma common variable, label the start of the cutting sequence with `NSTRT`, and immediately before `M30` increment the counter and conditionally `GOTO NSTRT`.
  - New property group `partLooping` ("Part Looping (Bar Feeder)", order 7, displayed between `programBehavior` and `output` -- `output` was bumped to order 8 to make room).
  - Properties (group `partLooping`):
    - `loopForMultipleParts` (boolean, default `false`) -- master enable.
    - `partCounterVariable` (integer 1-200, default `19`) -- common variable used as the completed-parts counter. Initialized to 0 at the top of the program; incremented at the end of each part.
    - `partsPerBar` (integer 0-9999, default `0`) -- hardcoded target. **`0` means "use the variable"** (see below). Any value > 0 is baked directly into the generated NC as a literal in the `IF[...] GOTO NSTRT` check.
    - `partsPerBarVariable` (integer 1-200, default `20`) -- common variable that holds the runtime target part count. Only used when `partsPerBar == 0`. The operator sets this at the control before running the program.
  - The `partsPerBar` / `partsPerBarVariable` pair is mutually exclusive: hardcoded value wins when nonzero, otherwise the variable is consulted at runtime. When `partsPerBar == 0` AND `partCounterVariable == partsPerBarVariable`, `onOpen` errors out (would otherwise infinite-loop or compare a counter against itself).
  - Output, top of program (end of `onOpen`, after the initial home retract): a blank line, then either `(SET V<n> TO NUMBER OF PARTS PER BAR)` (variable mode) or `(PROGRAM WILL LOOP <n> TIMES)` (hardcoded mode), then `V<counter>=0`, then `NSTRT (PART LOOP START)`. The `NSTRT` line carries an inline comment so the label is never bare (Okuma OSP accepts bare labels in practice, but every label in the reference hand-written program has at least one token after it).
  - Output, end of program (in `onClose`, between the final `onCommand(COMMAND_OPEN_DOOR)` and `M30`): `V<counter>=V<counter>+1`, then `IF[V<counter> LT <target>] GOTO NSTRT` where `<target>` is either the literal integer or `V<n>`.
  - Loop body extent: everything emitted AFTER `NSTRT` (which is at the end of `onOpen`) and BEFORE the `V<counter>+1` line is re-executed each iteration. That's all section NC. Excluded from the loop (run once per program): `O####` program header, `(comment)` lines, machine/tool list, `G90 G80`, `G50 S<max>`, chip-conveyor start, initial home retract. Excluded from the loop at the end: counter increment, GOTO, `M30`. Spindle / coolant / WCS / tool state is re-established by each iteration's first section as normal -- the machine ends each iteration at home with spindle stopped, so the next iteration's first section starts from a known state.
  - Marker comments: `// CUSTOM: part-loop mode group (bar feeder)` (groupDefinitions), `// CUSTOM: part-loop mode (bar feeder)` (property block), `// CUSTOM: part-loop mode -- initialize the completed-parts counter` (`onOpen` site), `// CUSTOM: part-loop mode -- increment the completed-parts counter` (`onClose` site).

- **Optional C-axis clamping (M146/M147).**
  - Property (group `spindle`):
    - `useCAxisClamping` (boolean, default `false`) -- when on, emit `M147` (LOCK_MULTI_AXIS / clamp C-axis brake) before non-simultaneous live-tool moves and `M146` (UNLOCK_MULTI_AXIS / release) afterwards. Most live-tool work on the LB15-II does not need the brake and the reference hand-written program never uses it, so the default is off.
  - Implementation: gates the two `onCommand` handlers (`COMMAND_LOCK_MULTI_AXIS` -> `M147`, `COMMAND_UNLOCK_MULTI_AXIS` -> `M146`) so they emit nothing when the property is off. Also gates the explicit `writeBlock(mFormat.format(getCode("UNLOCK_MULTI_AXIS", ...)))` inside the part-eject routine in `onClose`. Gating the `onCommand` handlers takes care of every indirect call site (section setup/wrap-up, polar/Y-axis transitions, multi-axis enable/disable) without having to touch each one individually.
  - Marker comment: `// CUSTOM: optional C-axis clamping (M146/M147)` (property block), `// CUSTOM: optional C-axis clamping -- skip M147 when disabled` / `// CUSTOM: optional C-axis clamping -- skip M146 when disabled` (the three gated emission sites).

- **Simplified M-cycles (drop D from G181, L from G183).**
  - Property (group `cycles`):
    - `useSimpleCycles` (boolean, default `false`) -- when on, omit the `D` (total depth) word from `G181` drilling cycles and the `L` (retract amount) word from `G183` deep-drilling / chip-breaking cycles. Use on older Okuma controls (LB15-II) that infer total depth from the `Z` target and retract amount from the peck depth.
  - Implementation: in `onCyclePoint`, the `"D" + spatialFormat.format(...)` word in the `drilling` case is replaced with a ternary that returns `""` when the property is on. Same treatment for the `"L" + ...` word in the `deep-drilling` and `chip-breaking` cases. The `D` word in `G183` cycles (peck depth) is preserved -- only `L` is suppressed there. Other cycles that share the D-word (`G182` counter-boring, `G184` tapping, `G189` reaming/boring) are not affected by this property; if those ever need similar treatment, add a separate property.
  - Marker comment: `// CUSTOM: simplified M-cycles (drop D from G181, L from G183)` (property block), `// CUSTOM: simplified G181 omits the D (total depth) word`, `// CUSTOM: simplified G183 omits the L (retract amount) word` (3 emission sites).

- **Move initial `G180` onto the program setup line.**
  - Stock post: in `onOpen`, the setup line emits `G90 G80` (absolute mode + turning-cycle cancel). The live-tool drilling-cycle cancel `G180` is not primed anywhere, so when the first `G181` drill cycle is posted, the preceding `writeCycleClearance(...)` -> `onCycleEnd()` call writes a stray `G180` on its own line immediately before the `G181` block. Functionally fine but reads as if `G180` belongs to the cycle.
  - Fix: in `onOpen`, change the setup-line `writeBlock(gAbsIncModal.format(90), gCycleModal.format(80))` to `writeBlock(gAbsIncModal.format(90), gCycleModal.format(80), gCycleModal.format(180))`. Two consecutive calls to `gCycleModal.format()` in one block work because each call updates the modal's internal current value before returning -- the first returns `"G80"` and sets current to 80, the second returns `"G180"` and sets current to 180. Net result: setup line reads `G90 G80 G180` and the modal is primed at 180 so the first `onCycleEnd` before the first `G181` is a modal no-op (no stray `G180` line). Subsequent `G181` -> `onCycleEnd` -> `G180` cycle-end emissions are unaffected (the modal flips 180 -> 181 -> 180 normally).
  - Marker comment: `// CUSTOM: include G180 (live-tool cycle cancel) alongside G80 on the setup` (the `onOpen` site).
